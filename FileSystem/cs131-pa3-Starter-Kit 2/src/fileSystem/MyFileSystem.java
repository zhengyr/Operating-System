...
public class MyFileSystem implements FileSystem {
    ...
    private DirectBlock getDirectBlock(int fd, MODE mode) {//FIXME!!!
        ...
        int levelOfIndirect = 0;
    	int countBlockNum = 9;
    	while(countBlockNum < blockNum){//calculate which level we should be
    		levelOfIndirect++;
    		countBlockNum = (int) (countBlockNum + Math.pow(IndirectBlock.COUNT, levelOfIndirect));
    	}
    	countBlockNum -= Math.pow(IndirectBlock.COUNT, levelOfIndirect); //the numbers of blocks before this level
    	int index = 0;
    	if(blockNum <= 9) index = blockNum;
    	else index = levelOfIndirect +9; //the index we should be in inode.ptr[]
    	boolean fresh = inode.ptr[index] == 0;
    	IndirectBlock newIndirect = new IndirectBlock();
    	if(fresh){//if the block is not created yet
            if(mode == MODE.r)//if we are reading
                return DirectBlock.hole;
            else if((inode.ptr[index] = freeMap.find()) == 0)//if we are writing and there is not enough space for us to write
                return null;
    	}else if(blockNum > 9){//if the block is created and we are trying to access an indirect block
    		disk.read(inode.ptr[index], newIndirect);
    	}
    	if(blockNum <= 9){//if we are accessing direct block
    		return new DirectBlock(disk, inode.ptr[index], blockOff, fresh);
    	}
    	//recursion starts for indirect blocks
        return getBlcokFromIndirect(blockNum, inode.ptr[index], blockOff, newIndirect, mode, levelOfIndirect, countBlockNum);

    }

    private DirectBlock getBlcokFromIndirect(int blockNum, int currentBlockNumber, int blockOff, IndirectBlock currentBlock, MODE mode,
			int levelOfIndirect, int countBlockNum) {
    	int blocksEachEntry = (int) Math.pow(IndirectBlock.COUNT, levelOfIndirect - 1);
    	int positionInCurrentBlock = (blockNum - countBlockNum - 1)/blocksEachEntry;
    	boolean fresh = currentBlock.ptr[positionInCurrentBlock] == 0;
    	IndirectBlock newIndirect = new IndirectBlock();
    	if(fresh){//base case check if our pointer is 0
            if(mode == MODE.r)
                return DirectBlock.hole;
            else if((currentBlock.ptr[positionInCurrentBlock] = freeMap.find()) == 0)
                return null;
            disk.write(currentBlockNumber, currentBlock);//if we are in write mode and we have free space, then we must write the new allocated space back to disk
    	}else{//if we our pointer is not 0
    		disk.read(currentBlock.ptr[positionInCurrentBlock], newIndirect);
    	}
    	if(levelOfIndirect == 1){
    		return new DirectBlock(disk, currentBlock.ptr[positionInCurrentBlock], blockOff, fresh);
    	}else{
    		int indexForNextLevel = blockNum % blocksEachEntry;
    		countBlockNum = (int) (countBlockNum - Math.pow(IndirectBlock.COUNT, levelOfIndirect - 1));//the number of blocks before next level
    		blockNum = indexForNextLevel + countBlockNum + 1; //block number for next level
    		return getBlcokFromIndirect(blockNum, currentBlock.ptr[positionInCurrentBlock], blockOff, newIndirect, mode, levelOfIndirect - 1, countBlockNum);
    	}
